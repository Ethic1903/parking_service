...
func (h *Handler) handleGetSpots(w http.ResponseWriter, r *http.Request) {
	...
	spots, err := h.service.SearchAvailableSpots(r.Context(), models.SearchFilter{
...
func (h *Handler) handleCreateBooking(w http.ResponseWriter, r *http.Request) {
	...
	booking, err := h.service.BookSpot(r.Context(), models.BookRequest{
	...
	if err != nil {
		switch {
		case errors.Is(err, repository.ErrSpotNotFound):
			h.writeError(w, http.StatusNotFound, err.Error())
		case errors.Is(err, repository.ErrSpotNotAvailable):
			h.writeError(w, http.StatusConflict, err.Error())
		case errors.Is(err, service.ErrInvalidTimeRange), errors.Is(err, service.ErrUserIDRequired):
